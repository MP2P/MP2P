...
//! Options for durability constraints
  ...
  public:
    ...
    DurabilityOptions(size_t persist_to, size_t replicate_to,
                      bool cap_max = true)
    {
...
//! Response received for durability operations
  ...
  public:
    ...
    bool on_master_storage() const
    {
      return u.resp.persisted_master ? true : false;
    }
...
//! This class is used to perform a single durability operation
  class DurabilityOperation
      : public Operation<Command<lcb_CMDENDURE>, EndureResponse>
  {
  public:
    //! Create the operation using the result of a previous operation
    //! @param op a @ref StoreOperation
    template<typename T>
    DurabilityOperation(const T& op) : Operation()
    {
...
//! BatchContext
  ...
  public:
    ...
    //!        be used further.
    DurabilityContext(Client& c, const DurabilityOptions& options,
                      Status& status) : h(c)
    {
