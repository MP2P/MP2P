...
if [ "$#" -eq 2 ] && [ -f "$1" ]; then
  ...
  echo
  echo Trying to connect to Raspberry Py 3...
  sshpass -p $PWD ssh root@$IP0 -p $PORT3 -T 'sh -s' < $1 &
  echo
  echo Trying to connect to Raspberry Py 4...
  sshpass -p $PWD ssh root@$IP4 -p $PORT4 -T 'sh -s' < $1 &
else
  if [ "$#" -eq 1 ]; then
    if [ -f "$1" ]; then
      echo Trying to connect to Raspberry Py 0...
      ...
      echo
      echo Trying to connect to Raspberry Py 3...
      sshpass -p $PWD ssh root@$IP3 -p $PORT3 -T 'sh -s' < $1
      echo
      echo Trying to connect to Raspberry Py 4...
      sshpass -p $PWD ssh root@$IP4 -p $PORT4 -T 'sh -s' < $1
    else
      ...
      echo
      echo Trying to connect to Raspberry Py 3...
      sshpass -p $PWD ssh root@$IP3 -p $PORT3 -T 'sh -s' < $( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )/command_list &
      echo
      echo Trying to connect to Raspberry Py 4...
      sshpass -p $PWD ssh root@$IP4 -p $PORT4 -T 'sh -s' < $( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )/command_list
      echo
      echo Trying to connect to Raspberry Py 1...
      sshpass -p $PWD ssh root@$IP1 -p $PORT1 -T 'sh -s' < $( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )/command_list &
      echo
